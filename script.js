...
let isPlay = false;
let isVolumeOn = true;
let nameOfCurrentTrack = document.querySelector(".player__currentTrack");
const tracks = [
  {
    title: "Aqua Caelestis",
    src: "../assets/sounds/Aqua Caelestis.mp3",
...
const listOfTracks = document.querySelector(".play-list");

function AddListOfTracks() {
  let fragmentWithListOfTracks = document.createDocumentFragment();
  tracks.forEach((el) => {
    const li = document.createElement("li");
    li.textContent = el.title;
    li.classList.add("play-item");
    fragmentWithListOfTracks.append(li);
  });
  listOfTracks.append(fragmentWithListOfTracks);
}
AddListOfTracks();

const playBtn = document.querySelector(".play-big");
const nextBtnPlayer = document.querySelector(".play-next");
const prevBtnPlayer = document.querySelector(".play-prev");
const audio = new Audio();
...
function playTrack() {
  changeCurrentTrack();
  let trackToPlay = tracks[currentTrack].src;
  isPlay = true;
  playBtn.classList.toggle("pause");
...
nextBtnPlayer.addEventListener("click", (e) => {
  currentTrack += 1;
  ...
  tracksInList[currentTrack].classList.add("item-active");
  if (isPlay) {
    let trackToPlay = tracks[currentTrack].src;
    audio.src = `${trackToPlay}`;
    audio.play();
...
prevBtnPlayer.addEventListener("click", (e) => {
  currentTrack -= 1;
  if (currentTrack < 0) {
...
function changeCurrentTrack() {
  nameOfCurrentTrack.textContent = tracks[currentTrack].title;
}
